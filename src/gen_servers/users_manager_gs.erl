...
-export([start/0, make_user_online/2, make_user_offline/1, make_users_offline_after_shutdown/0, ntf_about_new_chat_if_online/2, ntf_about_new_message_if_online/2, broadcast_msgs_have_read/2, ntf_about_new_group_member/2, ntf_about_group_invitation/3]).
...
handle_cast({ntf_about_new_group_member, ChatID, NewMemberID}, State) ->
  ...
  lists:foreach(
    fun(UserID) ->
      case db_gen_server:prepared_query(Query, [ChatID, UserID, NewMemberID, UserID, ChatID]) of
        {ok, _, [[1]]} ->
          Pid = maps:get(UserID, State),
